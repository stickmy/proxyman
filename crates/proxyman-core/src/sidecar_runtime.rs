...
use crate::{
    ...
    core_api::{
        ...
        upstream_tls as upstream_tls_api,
    },
...
impl SidecarRuntime {
    pub fn new() -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            session_store: InMemorySessionStore::with_app_rules_for_replay(),
        })
    }
    ...
    pub async fn handle_core_command(
        ...
        let result = match method {
            ...
            "upstreamTls.status" => handle_upstream_tls_status(params),
            "upstreamTls.update" => handle_upstream_tls_update(params),
            "systemProxy.enable" => {
...
fn handle_upstream_tls_status(params: Value) -> Result<Value, String> {
    let request =
        parse_params::<upstream_tls_api::UpstreamTlsStatusRequest>("upstreamTls.status", params)?;
    upstream_tls_api::validate_upstream_tls_status_request(&request)?;
    let ignore_verification = upstream_tls::read_ignore_upstream_tls_verification()?;
    to_json_value(upstream_tls_api::upstream_tls_status_response(
        ignore_verification,
    ))
}

fn handle_upstream_tls_update(params: Value) -> Result<Value, String> {
    let request =
        parse_params::<upstream_tls_api::UpdateUpstreamTlsRequest>("upstreamTls.update", params)?;
    upstream_tls_api::validate_update_upstream_tls_request(&request)?;
    let ignore_verification =
        upstream_tls::write_ignore_upstream_tls_verification(request.ignore_verification)?;
    to_json_value(upstream_tls_api::upstream_tls_status_response(
        ignore_verification,
    ))
}
