...
use hyper::{
    header::Entry, server::conn::Http, service::service_fn, upgrade::Upgraded, Body, Request,
    Response,
};
use snafu::ResultExt;
use tokio::{
...
use crate::{
    ...
    upstream_tls::UpstreamHttpClient,
};
...
impl<CA, P> Clone for Tunnel<CA, P>
where
    P: Clone,
{
    fn clone(&self) -> Self {
    ...
    P: processor::HttpProcessor + std::fmt::Debug,
{
    ...
    async fn handle_websocket(
        ...
        let connector = crate::upstream_tls::websocket_connector().map_err(|err| {
            tungstenite::Error::Tls(tungstenite::error::TlsError::Native(err))
        })?;
