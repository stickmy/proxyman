...
use crate::processors::{
    ...
    processor::HttpProcessor as ProcessorRunner,
};
use crate::upstream_tls::UpstreamHttpClient;
...
pub(crate) struct InMemorySessionStore {
    ...
    replay_rules: ReplayRuleSource,
}

impl InMemorySessionStore {
    pub(crate) fn with_app_rules_for_replay() -> Self {
        Self {
            state: Arc::default(),
            replay_rules: ReplayRuleSource::AppDir,
        }
    }

    #[cfg(test)]
    fn with_replay_processor(processor: HttpProcessor) -> Self {
        Self {
            state: Arc::default(),
            replay_rules: ReplayRuleSource::Static(processor),
        }
    }
}

#[derive(Debug, Clone, Default)]
enum ReplayRuleSource {
    #[default]
    None,
    #[cfg(test)]
...
impl ReplayRuleSource {
    fn load_processor(&self) -> Option<HttpProcessor> {
        match self {
            ReplayRuleSource::None => None,
            #[cfg(test)]
            ReplayRuleSource::Static(processor) => Some(processor.clone()),
            ReplayRuleSource::AppDir => Some(HttpProcessor::new(read_processors_from_appdir())),
        }
    }
}
...
impl SessionStore for InMemorySessionStore {
    ...
    async fn replay_request(
        ...
        send_replay_request(request, self.replay_rules.load_processor()).await
    }
}
...
async fn send_replay_request(
    ...
    processor: Option<HttpProcessor>,
) -> Result<SessionReplayResult, String> {
    let response = if let Some(processor) = processor {
        let processed = processor.process_request(request).await;
        if let Some(response) = processed.res {
            return replay_response_result(response).await;
        }

        let request_uri = processed.req.uri().clone();
        let response = send_replay_http_request(processed.req).await?;
        processor.process_response(&request_uri, response).await
    } else {
        send_replay_http_request(request).await?
    };

    replay_response_result(response).await
}

async fn send_replay_http_request(request: Request<Body>) -> Result<Response<Body>, String> {
    let client = UpstreamHttpClient::new()
        .map_err(|err| format!("Create replay upstream TLS client failed: {err}"))?;
    let response = client
        .request(request)
        .await
...
async fn replay_response_result(response: Response<Body>) -> Result<SessionReplayResult, String> {
...
mod tests {
    ...
    use crate::core_api::rules::parse_typed_rules_dsl;
    use crate::processors::{
        http_processor::typed_rules::TypedRuleProcessor, processor_pack::ProcessorPack,
    };
    ...
    #[tokio::test]
    async fn in_memory_session_replay_applies_request_rules_before_network() {
        let mut pack = ProcessorPack::new("replay-rules".to_string(), true);
        ...
        let store = InMemorySessionStore::with_replay_processor(HttpProcessor::new(vec![pack]));
        store
            .apply_event(CoreEventEnvelope {
                ...
                event: CoreEvent::RequestHead(RequestHeadEvent {
                    ...
                    uri: "http://blocked.example.test/asset.js".to_string(),
        ...
        let replay = store
            .replay_request("exchange-1", ReplayEdit::default())
            .await
            .expect("replay should use rule response");

        assert_eq!(replay.status, 451);
        assert_eq!(replay.body, "blocked-by-rule");
    }
