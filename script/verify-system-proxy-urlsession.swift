#!/usr/bin/env swift
// Verifies macOS system proxy behavior through Foundation URLSession.
// curl can bypass or override proxy settings, so this checks the same stack used by native apps.
import Darwin
import Foundation
